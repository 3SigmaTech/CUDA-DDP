...
namespace details {
    ...
    _CG_STATIC_QUALIFIER constexpr unsigned int scratch_sync_memory_size(unsigned int max_block_size) {
        // One barrier per possible size of the group rounded up to multiple of 4.
        return 8 * sizeof(details::barrier_t);
    }

    _CG_STATIC_QUALIFIER constexpr unsigned int scratch_collectives_memory_size(unsigned int communication_size, unsigned int max_block_size) {
...
private:
    ...
    _CG_QUALIFIER T* get_reduction_location(unsigned int warp_id) const {
        ...
        return reinterpret_cast<T*>(&tile_memory->memory[sync_mem_size + reduction_id * communication_size]);
    }
    ...
    _CG_QUALIFIER TyVal reduction_scheme_large_impl(
        ...
        if (warp.meta_group_rank() == 0) {
            ...
            // Thread 0 in reduction_threads set as inactive to not overwrite warp 0 warp_result.
            ...
            details::copy_channel<numWarps> partial_results_channel{get_reduction_location<char>(), sync_location, (size_t) communication_size};
