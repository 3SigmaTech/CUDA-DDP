...
QUALIFIERS double curand_MRG32k3a (curandStateMRG32k3a_t *state)
{
    ...
    p1 = __curand_umad(a12, state->s1[1], a13n * (unsigned long long)(m1 - state->s1[0]));

    p1 = (p1 & 0xffffffff) + (p1 >> 32) * m1c;
    if (p1 >= m1) p1 -= m1;

    state->s1[0] = state->s1[1]; state->s1[1] = state->s1[2]; state->s1[2] = p1;
    p2 = __curand_umad(a21, state->s2[2], a23n * (unsigned long long)(m2 - state->s2[0]));

    p2 = (p2 & 0xffffffff) + (p2 >> 32) * m2c;
    p2 = (p2 & 0xffffffff) + (p2 >> 32) * m2c;
    if (p2 >= m2) p2 -= m2;
